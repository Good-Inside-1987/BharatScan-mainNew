...
- Re-imported project set up again on 2026-07-18: Node module upgraded `nodejs-20` → `nodejs-22` (reset by re-import), ran `pnpm install`, configured `Backend API server` (port 3001, console) and `Frontend` (PORT=5000, port 5000, webview) workflows. Both verified running cleanly; app loads to the API key screen correctly.
